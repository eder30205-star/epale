            ...
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  {post.name!=="Tu"&&post.name!==userName&&onFollow?(<button onClick={function(){onFollow(post.name);}} style={{padding:"4px 12px",borderRadius:100,border:"1.5px solid "+(following.includes(post.name)?C.border:C.blue),background:"transparent",color:following.includes(post.name)?C.muted:C.blue,fontFamily:"'Inter',sans-serif",fontSize:11,fontWeight:700,cursor:"pointer"}}>{following.includes(post.name)?TR.siguiendo:TR.seguir}</button>):null}
    ...
    <div style={{position:"fixed",inset:0,zIndex:300,background:C.bg,maxWidth:480,margin:"0 auto",overflowY:"auto"}}>
      ...
      <div style={{padding:"24px 20px 60px"}}>
        ...
        {[["NOMBRE","Tu nombre",name,setName,false],["USUARIO","tu_usuario",username,function(v){setUsername(v.toLowerCase());},true]].map(function(item,i){
          ...
          return (<div key={i} style={{marginBottom:18}}><div style={{fontSize:10,fontFamily:"'Inter',sans-serif",color:C.muted,marginBottom:7,letterSpacing:1}}>{label}</div><div style={{position:"relative"}}>{isUser?<span style={{position:"absolute",left:14,top:10,color:C.muted,fontFamily:"'Inter',sans-serif",fontSize:15}}>@</span>:null}<input value={val} onChange={function(e){set(e.target.value);}} placeholder={ph} style={{width:"100%",padding:"13px 16px 13px "+(isUser?"30px":"16px"),background:C.card,border:"1.5px solid "+(val?C.blue:C.border),borderRadius:14,color:C.text,fontFamily:"'Inter',sans-serif",fontSize:15,outline:"none",boxSizing:"border-box"}}/></div></div>);
        })}
...
function AuthLogin(props) {
  ...
  return (<div style={{flex:1,padding:"22px 20px 32px",background:C.bg}}><div style={{fontSize:20,fontFamily:"'Syne',sans-serif",color:C.text,marginBottom:4,fontWeight:700}}>Bienvenido de vuelta</div><div style={{fontSize:13,color:C.muted,fontFamily:"'Inter',sans-serif",marginBottom:22}}>Entra para conectarte con tu gente</div><div style={{marginBottom:14}}><div style={{fontSize:10,fontFamily:"'Inter',sans-serif",color:C.muted,marginBottom:7,letterSpacing:1}}>CORREO</div><input value={email} onChange={function(e){setEmail(e.target.value);}} placeholder="tu@correo.com" type="email" style={{width:"100%",padding:"13px 16px",background:C.card,border:"1.5px solid "+(email?C.blue:C.border),borderRadius:12,color:C.text,fontFamily:"'Inter',sans-serif",fontSize:15,outline:"none",boxSizing:"border-box"}}/></div><div style={{marginBottom:8}}><div style={{fontSize:10,fontFamily:"'Inter',sans-serif",color:C.muted,marginBottom:7,letterSpacing:1}}>CONTRASENA</div><div style={{position:"relative"}}><input value={password} onChange={function(e){setPassword(e.target.value);}} type={showPass?"text":"password"} placeholder="Tu contrasena" style={{width:"100%",padding:"13px 46px 13px 16px",background:C.card,border:"1.5px solid "+(password?C.blue:C.border),borderRadius:12,color:C.text,fontFamily:"'Inter',sans-serif",fontSize:15,outline:"none",boxSizing:"border-box"}}/><button onClick={function(){setShowPass(function(s){return !s;});}} style={{position:"absolute",right:14,top:10,background:"none",border:"none",cursor:"pointer",fontSize:18}}>{ICONS.eye}</button></div></div><div style={{textAlign:"right",marginBottom:20}}><button onClick={function(){ var em=email; if(!em){setError("Ingresa tu correo primero");return;} api.resetPassword(em).then(function(){setError("Correo de recuperacion enviado!");}).catch(function(){setError("Error al enviar correo");}); }} style={{background:"none",border:"none",cursor:"pointer",color:"#0066ff",fontFamily:"'Inter',sans-serif",fontSize:11}}>Olvidaste tu contrasena?</button></div>{error?<div style={{background:"#3a1a1a",border:"1px solid #ff6b6b",borderRadius:10,padding:"9px 13px",marginBottom:13,fontSize:13,color:"#ff2d2d",fontFamily:"'Inter',sans-serif"}}>{error}</div>:null}<button onClick={go} style={{width:"100%",padding:"14px",background:"#ffcc00",color:C.text,border:"none",borderRadius:100,cursor:"pointer",fontFamily:"'Inter',sans-serif",fontSize:14,fontWeight:700}}>{loading?"Entrando...":"Entrar"}</button></div>);
}
...
function AuthStep1(props) {
  ...
  return (<div style={{flex:1,padding:"22px 20px 32px",background:C.bg}}><div style={{fontSize:20,fontFamily:"'Syne',sans-serif",color:C.text,marginBottom:16,fontWeight:700}}>Crea tu cuenta</div><div style={{marginBottom:14}}><div style={{fontSize:10,fontFamily:"'Inter',sans-serif",color:C.muted,marginBottom:7,letterSpacing:1}}>CORREO</div><input value={email} onChange={function(e){setEmail(e.target.value);}} type="email" placeholder="tu@correo.com" style={{width:"100%",padding:"13px 16px",background:C.card,border:"1.5px solid "+(email?C.blue:C.border),borderRadius:12,color:C.text,fontFamily:"'Inter',sans-serif",fontSize:15,outline:"none",boxSizing:"border-box"}}/></div><div style={{marginBottom:14}}><div style={{fontSize:10,fontFamily:"'Inter',sans-serif",color:C.muted,marginBottom:7,letterSpacing:1}}>CONTRASENA</div><div style={{position:"relative"}}><input value={password} onChange={function(e){setPassword(e.target.value);}} type={showPass?"text":"password"} placeholder="Minimo 6 caracteres" style={{width:"100%",padding:"13px 46px 13px 16px",background:C.card,border:"1.5px solid "+(password?C.blue:C.border),borderRadius:12,color:C.text,fontFamily:"'Inter',sans-serif",fontSize:15,outline:"none",boxSizing:"border-box"}}/><button onClick={function(){setShowPass(function(s){return !s;});}} style={{position:"absolute",right:14,top:10,background:"none",border:"none",cursor:"pointer",fontSize:18}}>{ICONS.eye}</button></div>{password?(<div><div style={{display:"flex",gap:3,marginTop:7}}>{[1,2,3,4,5].map(function(i){return <div key={i} style={{flex:1,height:3,borderRadius:2,background:i<=str?STR_COLORS[str]:C.border}}/>;})}</div><div style={{fontSize:11,color:STR_COLORS[str],fontFamily:"'Inter',sans-serif",marginTop:3}}>{STR_LABELS[str]}</div></div>):null}</div><div style={{marginBottom:20}}><div style={{fontSize:10,fontFamily:"'Inter',sans-serif",color:C.muted,marginBottom:7,letterSpacing:1}}>CONFIRMAR CONTRASENA</div><input value={password2} onChange={function(e){setPassword2(e.target.value);}} type="password" placeholder="Repite tu contrasena" style={{width:"100%",padding:"13px 16px",background:C.card,border:"1.5px solid "+(password2?(password2===password?"#1a7a3c":"#ff2d2d"):C.border),borderRadius:12,color:C.text,fontFamily:"'Inter',sans-serif",fontSize:15,outline:"none",boxSizing:"border-box"}}/></div>{error?<div style={{background:"#3a1a1a",border:"1px solid #ff6b6b",borderRadius:10,padding:"9px 13px",marginBottom:13,fontSize:13,color:"#ff2d2d",fontFamily:"'Inter',sans-serif"}}>{error}</div>:null}<button onClick={next} style={{width:"100%",padding:"14px",background:"#ffcc00",color:C.text,border:"none",borderRadius:100,cursor:"pointer",fontFamily:"'Inter',sans-serif",fontSize:14,fontWeight:700}}>Siguiente</button></div>);
}
